...
export default function ApplicationsApp({ workspace, onWorkspace, onSignOut }) {
  ...
  return <div style={{ minHeight:"100vh",background:`${grad.aurora},${c.sand}`,color:c.charcoal,fontFamily:FONT }}><style>{`
    ...
    @media(max-width:560px){.approval-shell{padding:12px 12px 70px;zoom:1}.approval-top{gap:8px}.approval-hero{align-items:flex-start;flex-direction:column;padding:18px}.approval-detail-grid{grid-template-columns:1fr}.approval-actions{display:grid;grid-template-columns:1fr}.approval-actions .approval-btn{width:100%}.approval-detail-head{flex-direction:column}.approval-gate-copy{padding:30px 20px}.approval-gate-card{padding:22px;margin:0 12px 14px}}
  `}</style><div className="approval-shell">
    <header className="approval-top"><div style={{ fontSize:22,fontWeight:800,letterSpacing:-.5 }}>Tico<span style={{ color:c.gold }}>Wild</span><span style={{ color:c.stone,fontWeight:700,fontSize:15,marginLeft:8 }}>CRM</span></div><WorkspaceSwitch workspace={workspace} onWorkspace={onWorkspace}/><div style={{ flex:1 }}/>{access?.member&&<button onClick={teamSignOut} className="approval-btn secondary"><KeyRound size={14}/> Disconnect approvals</button>}<button onClick={fullSignOut} className="approval-btn secondary"><LogOut size={14}/> Sign out</button></header>
