...
export default function OperatorsApp({ workspace, onWorkspace, onSignOut }) {
  ...
  const filtered = useMemo(() => {
    ...
    return list;
  }, [operators, query, regionFilter, categoryFilter, stageFilter]);
  ...
  return (
    <div style={{ minHeight: "100vh", background: c.sand, color: c.charcoal, fontFamily: FONT }}>
      <style>{`
        .ops-wrap { max-width: 1440px; margin: 0 auto; padding: 18px clamp(12px, 2.5vw, 28px) 90px; }
        .ops-topbar { display: flex; align-items: center; gap: 12px; flex-wrap: wrap; }
        .ops-stats { display: grid; grid-template-columns: repeat(auto-fit, minmax(140px, 1fr)); gap: 10px; margin: 16px 0 12px; }
        .ops-row:hover { background: rgba(255,255,255,.04); }
        .ops-kanban { display: flex; gap: 12px; overflow-x: auto; padding-bottom: 18px; }
        ...
        @media (max-width: 760px) {
          ...
          .ops-hide-mobile { display: none !important; }
      ...
      <div className="ops-wrap">
        ...
        <div className="ops-stats">
          {[
            { label: "Operators", value: stats.total, col: c.teal },
            { label: "In motion", value: stats.inMotion, col: c.blue },
            { label: "Active partners", value: stats.active, col: "#34D399" },
            { label: "Outreach due", value: stats.due, col: stats.due ? "#F87171" : c.stone },
            { label: "Tours priced", value: stats.tours, col: c.gold },
          ].map((s) => (
            <div key={s.label} style={{ padding: "12px 16px", borderRadius: radius.md, border: `1px solid ${c.line}`, background: c.white, boxShadow: shadow.sm }}>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center", marginBottom: 14 }}>
          ...
          {(query || stageFilter || regionFilter || categoryFilter) && (
            <button onClick={() => { setQuery(""); setStageFilter(""); setRegionFilter(""); setCategoryFilter(""); }}
        ...
        {view === "directory" && (
          <>
            ...
            <DirectoryTable operators={filtered} onOpen={setSelectedId} onStage={setStage} onLog={logTouch} />
          </>
        )}
...
// ── Directory table ───────────────────────────────────────────────────────────
function DirectoryTable({ operators, onOpen, onStage, onLog }) {
  ...
  if (!operators.length) {
    return <div style={{ padding: 40, textAlign: "center", color: c.stone, background: c.white, borderRadius: radius.md, border: `1px solid ${c.line}` }}>No operators match.</div>;
  }
  return (
    <div style={{ borderRadius: radius.md, border: `1px solid ${c.line}`, background: c.white, boxShadow: shadow.sm, overflow: "auto", maxHeight: "68vh" }}>
      <table style={{ borderCollapse: "collapse", width: "100%", minWidth: 900 }}>
        <thead style={{ position: "sticky", top: 0, zIndex: 2, background: c.canvas2 }}>
          <tr>
            {["Operator", "Stage", "Follow-up", "Regions", "Categories", "Tours", "Take rate", "Checklist", "Verified"].map((h) => (
          ...
          {operators.map((op) => {
            ...
            return (
              <tr key={op.id} className="ops-row" onClick={() => onOpen(op.id)} style={{ cursor: "pointer" }}>
                <td style={cell}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    <div>
                      ...
                      <div style={{ color: c.stone, fontSize: 11.5 }}>{op.contactType || (op.custom ? "Added manually" : "")}</div>
                    </div>
                ...
                <td style={{ ...cell, maxWidth: 220, overflow: "hidden", textOverflow: "ellipsis" }} title={op.categories.join(", ")}>
                  {op.categories.slice(0, 3).join(", ")}{op.categories.length > 3 ? ` +${op.categories.length - 3}` : ""}
                </td>
                <td style={{ ...cell, fontFamily: "var(--biz-font-mono, monospace)", fontWeight: 700 }}>{op.tourCount || "—"}</td>
...
function AddOperatorModal({ onClose, onSave }) {
  const [f, setF] = useState({ name: "", regions: "", categories: "", phone: "", whatsapp: "", email: "", website: "" });
