...
export function Nav({ page, go, tripCount, openTrip }) {
  ...
  return (
    <div style={{ position: "sticky", top: 0, zIndex: 60, background: "rgba(255,253,248,.85)", backdropFilter: "blur(12px)", borderBottom: "1px solid rgba(0,0,0,.06)" }}>
      <div style={{ maxWidth: 1180, margin: "0 auto", padding: "12px 20px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <button onClick={openTrip} style={{ position: "relative", background: "#fff", border: "1px solid rgba(0,0,0,.1)", cursor: "pointer", width: 40, height: 40, borderRadius: 999, display: "flex", alignItems: "center", justifyContent: "center" }}>
      ...
      {open && (
        <div style={{ borderTop: "1px solid rgba(0,0,0,.06)", padding: 12, display: "flex", flexDirection: "column", gap: 2, background: c.white }}>
          {LINKS.map(([id, label]) => (
