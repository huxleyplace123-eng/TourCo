...
import { ticoAnswer, ticoGreeting, ticoPageSuggestions } from "../intelligence/ticoChat.js";
...
export function TicoDock({ page, go, lift = false, trip = [] }) {
  const firstGreeting = ticoGreeting(page, trip.length);
  const [open, setOpen] = useState(false);
  const [input, setInput] = useState("");
  const [msgs, setMsgs] = useState([{ from: "tico", ...firstGreeting }]);
  const bodyRef = useRef(null);
  const suggestions = ticoPageSuggestions(page);

  // A new page gets its own concise welcome and useful next steps. The dock
  // still never auto-opens over the page.
  useEffect(() => {
    setOpen(false);
  ...
  const ask = (text, guidedAnswer = null) => {
    const q = (text ?? input).trim();
    if (!q) return;
    const a = guidedAnswer || ticoAnswer(q);
    setMsgs((m) => [...m, { from: "me", text: q }, { from: "tico", text: a.answer || a.text, action: a.action, mood: a.mood || "happy" }]);
    setInput("");
  };
  ...
  return (
    <div style={{ position: "fixed", right: 18, bottom: lift ? 86 : 18, zIndex: 60, display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 10, transition: "bottom .25s ease" }}>
        ...
        <div style={{ ...glass, background: "rgba(10,20,40,.95)", borderRadius: 20, width: "min(340px, calc(100vw - 32px))", boxShadow: "0 30px 70px -28px rgba(0,0,0,.95)", animation: "ticoPop .28s cubic-bezier(.2,.7,.2,1) both", overflow: "hidden", display: "flex", flexDirection: "column", maxHeight: "min(560px, calc(100vh - 120px))" }}>
          ...
          {msgs.length <= 1 && (
            <div style={{ display: "flex", flexWrap: "wrap", gap: 6, padding: "0 14px 10px" }}>
              {suggestions.map((s) => (
                <button key={s.label} onClick={() => ask(s.label, s)} style={{ background: "rgba(34,211,238,.1)", border: "1px solid rgba(34,211,238,.28)", color: c.teal, fontSize: 11.5, fontWeight: 700, padding: "6px 10px", borderRadius: 999, cursor: "pointer" }}>{s.label}</button>
              ))}
            </div>
          )}
          ...
          <div style={{ display: "flex", gap: 8, padding: "10px 12px", borderTop: `1px solid ${c.line}` }}>
            <input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && ask()}
              placeholder="Ask Rico anything…" aria-label="Ask Rico anything" style={{ flex: 1, background: "rgba(255,255,255,.06)", border: `1px solid ${c.line}`, borderRadius: 12, padding: "10px 12px", color: "#fff", fontSize: 13.5, outline: "none" }} />
      ...
      <button onClick={() => setOpen((o) => !o)} aria-label="Chat with Rico the Tico"
